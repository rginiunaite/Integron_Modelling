...
c = 1; % cell pointer in current array
 ...
 % Death check
    ...
    % Mutation 
    c = 42; % XXX Temporary -To be removed XXX
    ...
    currPopArr(c).FunctIntegrase = 1; % XXX Temporary - To be removed XXX
    % ---------------------------------------------------------------------
    % Reshuffle
    ...
    doesReshufflingOccur = and((currPopArr(c).FunctIntegrase == 1),(rand(1)<rho));
    ...
    if (doesReshufflingOccur==1) 
        ...
        numberGenesInCasette = length(currPopArr(c).Gentotype);
        ...
        newGenotype = exciseCassette(currPopArr(c).Gentotype, cassetteIdxToExcise, k);
        ...
        if (doesReinsertionOccur == 1)
            excisedCassette = currPopArr(c).Gentotype(cassetteIdxToExcise); % Identity of the excised cassette
