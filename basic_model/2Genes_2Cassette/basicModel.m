...
% ======================================================================
%% Model Parameters
n = 2;          % Number of different gene cassettes
k = 2;          % Number of cassettes in operon
nStressors = 2; % Number of different stressors
...
while tEnd<T
    ...
    for s = 1:nStressors
        ...
        meanTimetoNextSwitch = meanTimetoNextSwitch + switchProbVec(s);
    end
    
    r = rand(1);
    waitingTime = 1/meanTimetoNextSwitch*log(1/r); % Time we stay ('wait') in the current environmental conditions
    ...
    switchProbVec = switchProbVec/meanTimetoNextSwitch; % Normalise so that they are true probabilities
...
figure(1)
clf
subplot(nGenTypes+1,1,1)
plot(tVec,SMat(:,1),'LineWidth',2,'LineStyle','-') 
hold on
...
totPopwithGene1InFirstPos = ResultsMat(:,2) + ResultsMat(:,4) + ResultsMat(:,(2+nGenTypes)) + ResultsMat(:,(4+nGenTypes));
totPopwithGene2InFirstPos = ResultsMat(:,3) + ResultsMat(:,5) + ResultsMat(:,(3+nGenTypes)) + ResultsMat(:,(5+nGenTypes));
